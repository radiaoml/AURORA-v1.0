...
def train_model():
    ...
    for epoch in range(10):
        ...
        for images, targets in dataloader:
            ...
            map_target = torch.tensor([targets['map']], dtype=torch.float).to(device)
            situation_target = torch.tensor([targets['tactical_situation']], dtype=torch.float).to(device)
            formation_target = torch.tensor([targets['formation']], dtype=torch.float).to(device)
            position_target = torch.tensor([targets['position_type']], dtype=torch.float).to(device)
            
            # Performance targets
            perf = targets['performance_metrics']
            perf_target = torch.tensor([
                perf['entry_rating'] / 5.0,
                perf['timing_gap'] / 5.0,
                perf['formation_score'],
                perf['planting_score'],
                perf['rotation_score'],
                perf['win_rate']
